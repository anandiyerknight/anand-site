> STATUS 2026-08-21: Built the B2B outbound offer as an interactive 3D system map page (public/automation-machine.html) in the live site's design; every control browser-verified on the first build, polish re-check half done (test browser crashed, page fine). Not published, live site untouched. | NEEDS-YOU: open public/automation-machine.html in your browser; yes/no to publish
...
- [2026-08-21] [anand-site] **automation-machine map page** → `public/automation-machine.html`, one self-contained static file (no build step; serving it live = putting it on main + `git push vercel main`). Design tokens were read from the LIVE homepage CSS, not the repo — **the repo sits on the unshipped `rebrand-tokens` branch, so "match the site design" must always start from `curl https://anandiyer.co.in` + its CSS file** (live is monochrome: `--color-cyan:#fff`, Geist 800 display, glass, 80px grid; fonts via Google Fonts Geist + Geist Mono). Content verbatim from `~/CODE/automation-machine-deck/content.py` (8 machines / 38 agents / statuses / masked SHEET rows — the same data behind the Miro board). Browser-verify harness: `scripts/checkui-automation-machine.py` (throwaway headless Brave, auto-restarts around the headless-wedge; findings that came out of it: pointer-capture click-swallowing, headless forces prefers-reduced-motion, backdrop-filter raster storm — cross-project details in global METHOD 2026-08-21 [browser/interactive-canvas]). AVOID: merging/pushing `rebrand-tokens` to publish this page (ships the unapproved rebrand); qlmanage/screenshot-only verdicts on this page (JS canvas).
...
Verification state (three states, honest): full real-browser control pass GREEN on the pre-polish build (150 checks — every node, panel, link, drag, zoom; log `logs/ui-check-2026-08-21.md`); after the visual polish (brighter wires, perimeter feedback arcs, intro fade) the re-run verified 62/150 all-passing before the throwaway test browser crashed. REMAINING: finish that re-run — `scripts/checkui-automation-machine.py` (run: `uv run --python 3.12 --with websocket-client python scripts/checkui-automation-machine.py`).
